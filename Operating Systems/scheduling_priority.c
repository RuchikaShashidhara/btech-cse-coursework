...
struct process {
        int job_num;            // Job ID
        int priority;           // Priority, lower priority is given higher priority
...
Process* new_process (int job_num, int priority, int burst_time); 

// Create new Process with job_num & burst_time and inserts into Process Queue with priority based on priority
void insert_process(Process** head, int job_num, int priority, int burst_time);
...
{ 
        ...
        int job_num, burst_time, priority, total_num_of_jobs;
        ...
        {                
                ...
                scanf("%d", &burst_time);
                // Input Priority
                printf(" Enter Priority of %d Process: ", job_num);
                scanf("%d", &priority);
                
                // Insert process information into Process Queue
                if (job_num == 1)
                        ps = new_process(job_num, priority, burst_time);
                else
                        insert_process(&ps, job_num, priority, burst_time);
        }
...
Process* new_process (int job_num, int priority, int burst_time) 
{
        ...
        // Store Job ID, Priority and Burst Time
...
void insert_process(Process** head, int job_num, int priority, int burst_time)
{
        ...
        Process* temp = new_process(job_num, priority, burst_time);
        
        //  If new process prioity is higher than the current head (first process) priority, insert to head of Process Queue
...
void calculate_process(Process** head, int total_num_of_jobs)
{
        ...
        while (curr != NULL)
        {
                ...
                curr -> burst_time, curr_waiting_time, curr_turnaround_time);
